...
class Signal(np.ndarray):
    ...
    Load a stereo audio file, downmix it to mono:

    >>> sig = Signal('tests/data/audio/stereo_sample.flac', num_channels=1)
    >>> sig
    Signal([ 36,  36,  32, ..., 555, 524, 495], dtype=int16)
    >>> sig.num_channels
    1
    ...
    >>> sig
    Signal([-2470, -2553, -2766, ...,   591,   517,   677], dtype=int16)
    >>> sig.sample_rate
    22050
    ...
    >>> sig
    Signal([-0.07611084, -0.07659912, -0.07580566, ...,  0.01586914,
            0.01998901,  0.01950073], dtype=float32)
    >>> sig.dtype
    dtype('float32')
...
class SignalProcessor(Processor):
    ...
    Examples
    --------
    Processor for loading the first two seconds of an audio file, resampling it
    to 22.05 kHz and downmixing it to mono:

    >>> proc = SignalProcessor(sample_rate=22050, num_channels=1, stop=2)
    >>> sig = proc.process('tests/data/audio/sample.wav')
    >>> sig
    Signal([-2470, -2553, -2766, ...,    44,  -173,  -265], dtype=int16)
    >>> sig.sample_rate
    22050
    >>> sig.num_channels
    1
...
class FramedSignal(object):
    ...
    Examples
    --------
    To chop a :class:`Signal` (or anything a :class:`Signal` can be instatiated
    from) into overlapping frames of size 2048 with adjacent frames being 441
    samples apart:
    ...
    >>> frames[10]
    Signal([-6156, -5645, -5901, ...,  2032,  -253,   671], dtype=int16)
    >>> frames.fps
    100.0

    Instead of passing a :class:`Signal` sinstance as the first argument,
    anything a :class:`Signal` can be instatiated from (e.g. a file name) can
    ...
    >>> np.array(frames[2:4])
    array([[    0,     0,     0, ..., -5430, -5316, -5405],
           [ 2215,  2281,  2184, ...,   323,   561,   653]], dtype=int16)
...
class FramedSignalProcessor(Processor):
    ...
    be instatiated from) into overlapping frames of size 2048, and a frame rate
    of 100 frames per second:

    >>> proc = FramedSignalProcessor(frame_size=2048, fps=100)
    >>> frames = proc.process('tests/data/audio/sample.wav')
    >>> frames  #doctest: +ELLIPSIS
    <madmom.audio.signal.FramedSignal object at 0x...>
    >>> frames[0]
    Signal([    0,     0,     0, ..., -4553, -4666, -4589], dtype=int16)
